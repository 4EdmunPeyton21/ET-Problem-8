...
async function run() {
    ...
    // Perform POST upload to /api/documents/upload using 'file' field
    const uploadRes = await postMultipart(
        'http://localhost:3010/api/documents/upload',
        testFile,
        'file', // Field name matches request contract
    ...
    if (uploadRes.status !== 202 || !uploadRes.body.documentId) {
        console.error('❌ Upload failed. Expected status 202 and a documentId.');
        process.exit(1);
    }

    const docId = uploadRes.body.documentId;
    console.log(`\nDocument successfully queued. Assigned ID: ${docId}`);
    ...
    while (!complete && attempts < maxAttempts) {
        attempts++;
        const statusRes = await getJson(`http://localhost:3010/api/documents/${docId}/status`);
        
        const { progress, status, entitiesExtracted, errors } = statusRes.body;
        ...
        if (status === 'completed' || status === 'failed') {
            ...
            if (status === 'completed') {
                ...
                console.error(`❌ Ingestion failed. Errors: ${errors.join(', ')}`);
            }
        } else {
    ...
    // 4. Retrieve metadata from Neo4j
    console.log('\n── Step 4: Retrieving Document Metadata from Neo4j ───────────');
    const metaRes = await getJson(`http://localhost:3010/api/documents/${docId}`);
    console.log('Neo4j Node Properties:\n', JSON.stringify(metaRes.body, null, 2));
